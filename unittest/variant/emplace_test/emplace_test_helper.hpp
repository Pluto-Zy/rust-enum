...
template <class Variant, std::size_t Index, class... Args>
struct has_index_emplace
    : decltype(has_index_emplace_impl<Variant, Index, Args...>(0)) { };

template <
    class Variant, class Type, class... Args,
    class = std::void_t<decltype(std::declval<Variant>().template emplace<Type>(
        std::declval<Args>()...))>>
...
struct has_type_emplace
    : decltype(has_type_emplace_impl<Variant, Type, Args...>(0)) { };
}  // namespace rust
