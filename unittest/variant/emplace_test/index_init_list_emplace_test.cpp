...
TEST(VariantTestIndexInitListEmplace, Deleted) {
    ...
    static_assert(has_index_emplace<variant<list&>, 0, list&>::value);
    {
...

        {
            ...
            static_assert(has_index_emplace<v, 0, list&>::value);
            static_assert(!has_index_emplace<v, 0, std::initializer_list<double>>::value);
...
TEST(VariantTestIndexInitListEmplace, BasicBehavior) {
    ...
    }
    {
        ...
        using v = variant<int, nothrow_move_constructible, nothrow_move_constructible>;
        v x;
        EXPECT_EQ(x.index(), 0);
        x.emplace<1>({ 1, 2, 3 });
        EXPECT_EQ(x.index(), 1);
