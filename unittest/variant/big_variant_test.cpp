...
TEST(VariantTestBigVariant, SizeAndAlignment) {
  ...
  static_assert(std::is_same<variant_index<0>::type, std::uint8_t>::value);
  static_assert(std::is_same<variant_index<1>::type, std::uint8_t>::value);
  static_assert(std::is_same<variant_index<u8_max>::type, std::uint8_t>::value);
  static_assert(
      std::is_same<variant_index<u8_max + 1>::type, std::uint16_t>::value);
  static_assert(
      std::is_same<variant_index<u16_max>::type, std::uint16_t>::value);
  static_assert(
      std::is_same<variant_index<u16_max + 1>::type, std::uint32_t>::value);
  static_assert(
      std::is_same<variant_index<u32_max>::type, std::uint32_t>::value);
