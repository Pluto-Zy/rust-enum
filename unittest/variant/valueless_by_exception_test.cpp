...
TEST(VariantTestValuelessByException, Basic) {
    {
        using v = variant<int>;
        v x1;
        static_assert(std::is_same<decltype(x1.valueless_by_exception()), bool>::value);
        static_assert(noexcept(x1.valueless_by_exception()));
        EXPECT_FALSE(x1.valueless_by_exception());

        constexpr v x2;
        static_assert(std::is_same<decltype(x2.valueless_by_exception()), bool>::value);
    ...
    }
    {
        ...
        static long data = 3;
        constexpr v x(data);
        static_assert(std::is_same<decltype(x.valueless_by_exception()), bool>::value);
